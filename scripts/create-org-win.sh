...
sf org create scratch --definition-file $scratchDef --alias $orgAlias --duration-days $orgDuration --set-default

echo "[2 / 7] Installing packages..."
# Nebula Logger
echo y | sf package install --wait 10 --security-type AdminsOnly --package 04t5Y0000027L98QAE --target-org $orgAlias
# Netomi Agent Assist
sf package install --wait 10 --security-type AdminsOnly --package 04tQQ0000000PxhYAE --target-org $orgAlias
echo
...
sf org assign permset --name ContactCenterAdminExternalTelephony --target-org $userName
sf project deploy start --target-org $orgAlias
echo

echo "[6 / 8] Assigning extra permissions sets to admin..."
sf org assign permset --name SAS_Customer_Claims --target-org $userName
echo

echo "[7 / 8] Pushing data..."
cd scripts/create-org-mock-data/
sf sfdmu:run --targetusername $orgAlias --sourceusername csvfile --quiet
cd ../../
echo
